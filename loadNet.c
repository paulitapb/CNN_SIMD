...
conv_kernels read_conv_kernels(FILE* file_ptr, long int *file_idx, int dim_in, int dim_out, int kernel_size){
    ...
    // Create kernel
	conv_kernels kernels = _mm_malloc(dim_in * sizeof(conv_kernels_row), 32);

	// For each image input
	for(int img_in = 0; img_in < dim_in; img_in++){

		// Create kernels row
		conv_kernels_row kernels_row = _mm_malloc(dim_out * sizeof(conv_kernel), 32);

		// For each image output
		for(int img_out = 0; img_out < dim_out; img_out++){

			// Create kernel for image_in image_out
			conv_kernel kernel = _mm_malloc(kernel_size * sizeof(conv_kernels_row),  32);

			// For each kernel row
			for(int i = 0; i < kernel_size; i++){

				// Create row
				conv_kernel_row kernel_row = _mm_malloc(kernel_size * sizeof(float), 32);
...
conv_bias read_conv_bias(FILE* file_ptr, long int *file_idx, int dim_out){
    ...
    // Create bias
	conv_bias bias = _mm_malloc(dim_out * sizeof(float), 32);
...
void load_dense(dense_struct *dense_layer, FILE *file_ptr, long int *file_idx){
    ...
    *file_idx += 2;
    ...
    //load weights
    dense_kernel weights = _mm_malloc(sizeof(dense_kernel_row)*(dense_layer->dim_out),  32); 
    for (int i = 0; i < (dense_layer->dim_out); i++)
    {
        dense_kernel_row r = _mm_malloc(sizeof(float)*(dense_layer->dim_in),  32); 
    ...
    *file_idx += 1;
    ...
    // load bias 
    dense_bias bias = _mm_malloc(sizeof(float)*(dense_layer->dim_out), 32); 
    for (int i = 0; i < dense_layer->dim_out; i++)
    {
